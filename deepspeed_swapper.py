...
from deepspeed.runtime.swap_tensor.utils import swap_in_tensors, swap_out_tensors, MIN_AIO_BYTES, AIO_ALIGNED_BYTES, print_object, SwapBufferPool
from .thread_safe_queue import ThreadSafeDeque
from .io_handle import NvmeHandle, PartitionedParamStatus
from .offload_utils import print_rank_0
from memory_profiler import profile


class ThreadSafeAsyncPartitionedParameterSwapper(object):
    @profile
    def __init__(self, ds_config, model_dtype):
        ...
        # thread local params stored in handles
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            ...
            print_object(
                obj=self, name='AsyncPartitionedParameterSwapper', exclude_list=exclude_list)
    ...
    def _configure_aio(self, ds_config):
        ...
        self.swap_folder = os.path.join(
            self.swap_config.nvme_path, 'zero_stage_3', f'{torch_dtype_string}params')
        shutil.rmtree(self.swap_folder, ignore_errors=True)
        os.makedirs(self.swap_folder, exist_ok=True)

        self.swap_element_size = torch.tensor(
        ...
        self.available_buffer_ids = ThreadSafeDeque([i for i in range(self.param_buffer_count)])
        self.reserved_buffer_ids = []
        
        acc = get_accelerator()
        
        self.buffers = [ torch.empty(self.aligned_elements_per_buffer, dtype=self.dtype, requires_grad=False, pin_memory = True)
                        for _ in range(self.param_buffer_count)
                        ]
        
        print(f"one buffer size: {self.buffers[0].numel() * self.buffers[0].element_size() / 1024 / 1024} MB")
        
        # print_rank_0(f"allocated {self.param_buffer_count} * {self.aligned_elements_per_buffer} buffers")
        ...
        for param in params:
            ...
            if param_id in self.id_to_path.keys():
                ...
                param_path = os.path.join(
                    self.swap_folder, f'{param_id}_param.tensor.swp')
    ...
    def _allocate_and_return_buffers_for_swap_in(self, handle: NvmeHandle, params):
        compute_buffers = []
        swap_buffers = []

        for param in params:
            param_id = param.ds_id
            assert param_id in self.param_id_to_numel.keys(
            ), f" Number of elements in param {param_id} is unknown"
            ...
            print(f"[SWAPPER] try allocate buffer for {param.ds_id}")
            buffer_id = self.available_buffer_ids.pop()
            print_rank_0(
                f"[SWAPPER] param {param.ds_id} allocs buffer {buffer_id}  ")
            
            self.param_id_to_buffer_id[param_id] = buffer_id
            aligned_swap_numel = self._io_aligned_numel(
                self.param_id_to_numel[param_id])
            swap_buffer = self.buffers[buffer_id].narrow(
                0, 0, aligned_swap_numel)
            self.param_id_to_swap_buffer[param_id] = swap_buffer
            compute_buffer = swap_buffer.narrow(
                0, 0, self.param_id_to_numel[param_id])
    ...
    def remove_partition_and_release_buffers(self, params):
        for param in params:
            ...
            if param_id in self.param_id_to_buffer_id.keys():
                ...
                print_rank_0(
                    f"[SWAPPER] param {param.ds_id} releases buffer id {buffer_id}  ")
    ...
    def _swap_out(self, handle: NvmeHandle, params, async_op=True):

        swap_out_paths = self._get_swap_paths(params)
        swap_out_params = self._get_swap_buffers(params)
        self._track_numel(params)

        swap_out_tensors(handle,
                         swap_out_params, swap_out_paths)

        handle.pending_writes += len(swap_out_params)
        handle.swap_out_params += params

        if not async_op:
            handle._synchronize_writes()

    # blocking swap out followed by releasing the memory buffers
    def swap_out_and_release(self, handle: NvmeHandle, params, async_op=False, force_buffer_release=False):
        if async_op:
        ...
        self._swap_out(handle, params, async_op=async_op)

    # book keeping function for inflight swap in
    def _update_inflight_swap_in(self, handle: NvmeHandle, params, swap_in_buffers, inflight_numel):
        handle.inflight_params.extend(params)
        handle.inflight_swap_in_buffers.extend(swap_in_buffers)
        handle.inflight_numel += inflight_numel

        for param in params:
            param.ds_tensor.status = PartitionedParamStatus.INFLIGHT

        handle.pending_reads += len(params)

    # assigns an in memory buffer and swaps in from nvme
    def swap_in(self, handle: NvmeHandle, params, async_op=True, swap_in_buffers=None):
        ...
        if swap_in_buffers is None:
            # print_rank_0("swap_in buffer is now empty, wait for other threads to return buffer...")
            ...
            compute_buffers, swap_in_buffers = self._allocate_and_return_buffers_for_swap_in(
                handle, params)
            inflight_numel = sum([t.numel() for t in compute_buffers])
        else:
            inflight_numel = sum([t.numel() for t in swap_in_buffers])

        swap_in_tensors(handle, swap_in_buffers, swap_in_paths)

        self._update_inflight_swap_in(handle, params, swap_in_buffers, inflight_numel)
    ...
    def swap_into_buffer(self, param, dest_buffer):
        ...
        if require_swap_buffer:
            assert len(
                self.available_buffer_ids) > 0, f"No buffer available to swap param {param.ds_id}."
            compute_buffers, swap_in_buffers = self._allocate_and_return_buffers_for_swap_in([
                                                                                             param])
    ...
    def get_buffer(self, param, numel):
        ...
        swap_buffer = self.buffers.narrow(
            0, int(buffer_id * self.aligned_elements_per_buffer), aligned_swap_numel)
    ...
    def reserve_partitioned_swap_space(self, partition_num_elems):
        aligned_numel = sum([self._io_aligned_numel(numel)
                            for numel in partition_num_elems])
        self.partitioned_swap_buffer = get_accelerator().pin_memory(
            torch.zeros(aligned_numel, device='cpu', dtype=self.dtype, requires_grad=False))
        self.partitioned_swap_pool = SwapBufferPool(
    ...
    def swap_out_partitioned_params(self, dst_fp16_params, src_fp32_params):
        ...
        fp16_swap_paths = self._get_swap_paths(
            dst_fp16_params, must_exist=True)
